...
CREATE TABLE IF NOT EXISTS TRANSACTION_HISTORY (
    ...
    CONSTRAINT CHK_STORE_OR_FACTORY_EXCLUSIVE
--  둘중 하나는 존재해야됨
    CHECK (
        (STORE_ID IS NOT NULL AND FACTORY_ID IS NULL) OR
        (STORE_ID IS NULL AND FACTORY_ID IS NOT NULL)
    )
);

ALTER TABLE TRANSACTION_HISTORY
    ADD COLUMN ACCOUNT_SALE_CODE BIGINT,
    ADD COLUMN TRANSACTION_HISTORY_NOTE VARCHAR(255);